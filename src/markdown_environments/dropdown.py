    ...
    def run(self, parent, blocks):
        ...
        if not self.SUMMARY_START_REGEX.match(blocks[1]):
            if self.is_thm:
                ...
                # `blocks = org_blocks` doesn't work; must mutate `blocks` instead of reassigning it
                blocks.clear()
        ...
        # find and remove summary ending delim if summary starting delim was present,
        # and extract element
        # (`summary_elem` is initialized outside the loop since the loop isn't guaranteed here 
        ...
        if has_summary:
            for i, block in enumerate(blocks):
                # if we haven't found summary ending delim but have found the overall dropdown
                # ending delim, then don't keep going; maybe the summary was omitted as it was
                # optional for theorems
        ...
        if not has_valid_summary:
            ...
            blocks.extend(org_blocks)
            return False
        # prepend thm heading (including default summary) to summary if applicable,
        # again outside the loop
        ...
        for i, block in enumerate(blocks):
            if self.end_pattern.search(block):
                ...
                if self.html_class != "" or self.type_opts.get("html_class") != "":
                    details_elem.set(
                        "class", f"{self.html_class} {self.type_opts.get('html_class')}"
                    )
...
class DropdownExtension(Extension):
    r"""
    A dropdown that can be toggled open or closed, with only a preview portion (`<summary>`)
    shown when closed.
    ...
    Important:
        The `summary` block must be placed at the *start* of the `dropdown` block, of course
        with blank lines before and after the `summary` block.
    """
    ...
    def __init__(self, **kwargs):
        ...
        arguments:
            ...
            - **html_class** (*str*) -- HTML `class` attribute to add to all dropdowns.
              Defaults to `""`.
            - **summary_html_class** (*str*) -- HTML `class` attribute to add to all dropdown
              summaries. Defaults to `""`.
            - **content_html_class** (*str*) -- HTML `class` attribute to add to all dropdown
        ...
        matched against, so it is effectively useless.) In addition, each type's value is itself
        a dictionary with the following possible options:

            - **html_class** (*str*) -- HTML `class` attribute to add to dropdowns of that type.
              Defaults to `""`.
        """
        ...
        self.config = {
            ...
            "is_thm": [
                False,
                (
                    "Whether to use theorem logic (e.g. heading); used only by `ThmExtension`. "
                    "Defaults to `False`."
                )
            ]
        }
    ...
    def extendMarkdown(self, md):
        md.parser.blockprocessors.register(
            DropdownProcessor(md.parser, **self.getConfigs()), "dropdown", 105
        )
